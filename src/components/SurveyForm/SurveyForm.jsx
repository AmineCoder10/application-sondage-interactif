...
export default function SurveyForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3001/Answers', { surveyId: id, responses: answers, age, gender });
            ...
            <form onSubmit={handleSubmit}>
              {surveyData.questions.map((question, index) => (
                <div key={index} className='demo'>
                  <div>
                    <p className='title-underline' style={{ color: "black", fontSize: "20px", fontWeight: "600", marginBottom: "20px" }}>{question.question}</p>
                  </div>
              ...
              <motion.button whileHover={{ scale: 1.08 }} className='collect-btn' type="submit" disabled={!allQuestionsAnswered}
                style={{ position: "absolute", bottom: "2%", right: "-1.8%", marginRight: "80px", marginTop: "20px", paddingRight: "20px", paddingTop: "8px", paddingLeft: "20px", paddingBottom: "8px" }}>Submit</motion.button>
            </form>

          </Col>
          <Col lg="4">

            <div className='selection' style={{ backgroundColor: "#d6e5fb", height: "40vh", borderRadius: "15px", marginBottom: "50px" }}>
